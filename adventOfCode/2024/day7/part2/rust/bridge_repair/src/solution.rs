...
// Changes in can_equation_be_true() method and added concatenate_two_numbers()
// ==============
...
impl Equation {
    ...
    /// of operators ('+' and '*' and '||') between the equation operands.
    fn can_equation_be_true(&self, current_cumulative_result: i64, operand_index: usize) -> bool {
        ...
        // RECURSION
        if      self.can_equation_be_true(
            current_cumulative_result + self.operands[operand_index], operand_index + 1) { true } // we try '+'
        else if self.can_equation_be_true(
            current_cumulative_result * self.operands[operand_index], operand_index + 1) { true } // we try '*'
        else if self.can_equation_be_true(
            concatenate_two_numbers(current_cumulative_result, self.operands[operand_index]), operand_index + 1) { true } // we try '||'
        else    { false }
    }
}

fn concatenate_two_numbers(number1: i64, number2: i64) -> i64 {

    let result_string: String = number1.to_string() + &number2.to_string();

    result_string.parse().expect("ERROR GETTING NUMBER FROM STRING!")
}
